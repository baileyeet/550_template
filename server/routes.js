...
const author = async function(req, res) {
  ...
  const name = 'Machiko Hirota';
  const pennKey = 'bhirota';
  ...
  connection.query(
    ...
    (err, data) => {
      if (err) {
        ...
        // Specify the shape of each object in the returned array
...
const top_albums = async function(req, res) {
  ...
  if (!page) {
    connection.query(
      `
      SELECT Albums.album_id, Albums.title, SUM(Songs.plays) AS total_plays
      FROM Albums
...
const search_songs = async function(req, res) {
  ...
  const sql = `
    ...
    WHERE
      ...
      energy <= ? AND
      valence >= ? AND
      valence <= ? ${explicit === 'true' ? '' : 'AND explicit = 0'}
    ORDER BY title ASC;
  `;

  const params = [
    `%${title}%`,
    parseInt(durationLow),
    parseInt(durationHigh),
    parseInt(playsLow),
    parseInt(playsHigh),
    parseFloat(danceabilityLow),
    parseFloat(danceabilityHigh),
    parseFloat(energyLow),
    parseFloat(energyHigh),
    parseFloat(valenceLow),
    parseFloat(valenceHigh),
  ];
